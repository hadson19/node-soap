...
WSDL.prototype.xmlToObject = function(options, callback) {
  var xml = null;
  var location = null;
    ...
    if (!objectName && top.name === 'Body' && name !== 'Fault') {
      ...
      if (!message) {
        try {
          ...
          // set default Prototype name
          var portType = portTypes[portTypeNames[0]];
          var method = portType.methods[name];

          if (location) {
            var services = self.definitions.services;

            Object.keys(services).forEach(function(serviceName) {
              Object.keys(services[serviceName].ports).forEach(function(portName) {
                if (services[serviceName].ports[portName].location === location) {
                  method = services[serviceName].ports[portName].binding.methods[name];
                }
              });
            });
          }
